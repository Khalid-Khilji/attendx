    ...
    pipeline = [
        ...
        {"$unwind": {"path": "$teacher", "preserveNullAndEmptyArrays": True}},
        {
            "$project": {
                ...
                "course_name": "$course.name", "batch_name": {"$ifNull": ["$batch.name", "Theory"]},
...
async def get_my_timetable(current_user):
    logged_in_teacher_id = current_user.get("user_id")
    pipeline = [
        {"$lookup": {"from": "course_teachers", "localField": "teacher_id", "foreignField": "_id", "as": "bridge"}},
        {"$unwind": "$bridge"},
        {"$match": {"bridge.teacher_id": logged_in_teacher_id, "is_active": True}},
        {"$lookup": {"from": "courses", "localField": "course_id", "foreignField": "_id", "as": "course"}},
        {"$unwind": "$course"},
        {"$lookup": {"from": "batches", "localField": "batch_id", "foreignField": "_id", "as": "batch"}},
        {"$unwind": {"path": "$batch", "preserveNullAndEmptyArrays": True}},
    ...
    pipeline = [
        {
            "$match": {
                ...
                "is_active": True,
                "$or": [{"batch_id": None}, {"batch_id": student_batch_id}]
            }
        },
        {"$lookup": {"from": "courses", "localField": "course_id", "foreignField": "_id", "as": "course"}},
        {"$unwind": "$course"},
        {
            "$project": {
                "_id": 1, "day_of_week": 1, "start_time": 1, "end_time": 1,
                "course_name": "$course.name", "is_batch_specific": {"$cond": [{"$gt": ["$batch_id", None]}, True, False]}
            }
        },
        {"$sort": {"day_of_week": 1, "start_time": 1}}
    ]
