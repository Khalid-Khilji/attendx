...
import { BookMarked, ArrowRight, ChevronDown, Award, Hash } from 'lucide-react';
import { getAllSemesters, getEnrollmentHistory, enrollStudent, promoteStudent, getSemBatches } from '../../api/index';
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} size="md">
            ...
            <AnimatePresence mode="wait">
                {activeEnrollment ? (
                    promoteMode ? (
                        <motion.form key="promote-form" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} onSubmit={(e) => { e.preventDefault(); promoteForm.handleSubmit(); }} className="space-y-6 px-2">
                            <div>
                                <label className={labelClass}>Target Semester</label>
                                <div className="relative group">
                                    <promoteForm.Field name="next_sem_id">
                                        {(f) => <select value={f.state.value} onChange={(e) => { f.handleChange(e.target.value); setSelectedSem(e.target.value); }} className={selectBaseClass} required>
                                            <option value="" disabled>SELECT SEMESTER</option>
                                            {semesters.filter(s => s._id !== activeEnrollment.sem_id).map(s => <option key={s._id} value={s._id}>SEMESTER {s.sem_number}</option>)}
                                        </select>}
                                    </promoteForm.Field>
                            ...
                            <div>
                                <label className={labelClass}>Assign Batch</label>
                                <div className="relative group">
                                    <promoteForm.Field name="next_batch_id">
                            ...
                            <div>
                                ...
                                <div className="relative group">
                                    <promoteForm.Field name="next_academic_year_id">
                                        {(f) => <select value={f.state.value} onChange={(e) => f.handleChange(e.target.value)} className={selectBaseClass} required>
                            ...
                            <div className="flex gap-3 pt-4">
                                ...
                                <Button type="submit" className="flex-2 h-14 rounded-2xl shadow-xl shadow-violet-500/10 font-black uppercase text-[11px]" isLoading={promoteMutation.isPending} icon={ArrowRight} iconPosition="right">Execute Promotion</Button>
                            </div>
                        </motion.form>
                    ) : (
                        <motion.div key="active-view" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="space-y-6 px-2">
                            <div className="bg-zinc-50 dark:bg-zinc-900 rounded-4xl p-8 border border-zinc-100 dark:border-zinc-800 text-center relative overflow-hidden">
                                <div className="absolute top-0 right-0 p-4 opacity-10"><Award size={80} className="text-violet-600" /></div>
                                ...
                                <div className="flex flex-col gap-2 items-center">
                                    <div className="inline-flex items-center gap-2 bg-white dark:bg-zinc-800 px-5 py-2.5 rounded-full shadow-sm border dark:border-zinc-700">
                                        ...
                                        <span className="text-xs font-black text-zinc-700 dark:text-zinc-200 tracking-widest uppercase">SEMESTER {semesters.find(s => s._id === activeEnrollment.sem_id)?.sem_number || '...'}</span>
                                    </div>
                                    {activeEnrollment.batch_id && (
                                        <div className="inline-flex items-center gap-2 bg-violet-50 dark:bg-violet-900/20 px-4 py-1.5 rounded-full border border-violet-100 dark:border-violet-800">
                                            <Hash size={10} className="text-violet-600" />
                                            <span className="text-[10px] font-black text-violet-600 uppercase">Batch Registered</span>
                                        </div>
                                    )}
                                </div>
                            </div>
                            <Button variant="primary" className="w-full h-14 rounded-2xl shadow-xl shadow-violet-500/20 font-black uppercase text-[11px]" onClick={() => { setPromoteMode(true); setSelectedSem(''); }} icon={ArrowRight} iconPosition="right">Initialize Promotion</Button>
                        </motion.div>
                    )
                ) : (
                    <motion.form key="enroll-form" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} onSubmit={(e) => { e.preventDefault(); enrollForm.handleSubmit(); }} className="space-y-6 px-2">
                        <div>
                            <label className={labelClass}>Target Department</label>
                            <div className="relative group">
                                <select value={selectedDept} onChange={(e) => { setSelectedDept(e.target.value); setSelectedSem(''); }} className={selectBaseClass} required>
                        ...
                        <div>
                            <label className={labelClass}>Academic Timeline</label>
                            <div className="relative group">
                        ...
                        <div className="flex gap-3 pt-4">
                            <Button variant="ghost" className="flex-1 h-14 rounded-2xl font-black uppercase text-[11px]" onClick={onClose}>Discard</Button>
                            <Button type="submit" className="flex-2 h-14 rounded-2xl shadow-xl shadow-violet-500/10 font-black uppercase text-[11px]" isLoading={enrollMutation.isPending}>Commit Enrollment</Button>
                        </div>
                    </motion.form>
                )}
            </AnimatePresence>
