...
import { Plus, Trash2, Calendar, LayoutGrid, ChevronDown, Hash } from 'lucide-react'
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query'
import { getSemTimetable, deleteSlot, getAllSemesters, getSemBatches } from '../../api/index'
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 pt-10 pb-20">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
          ...
          <div className="relative group">
            <select value={selectedBatch} onChange={(e) => setSelectedBatch(e.target.value)} disabled={!selectedSem} className={`${selectClass} disabled:opacity-50`}>
              <option value="">All Batches (Theory + Lab)</option>
        ...
        {!selectedSem ? (
          <div className="py-32 text-center flex flex-col items-center justify-center opacity-30">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.4em]">Select level to load schedule</p>
          </div>
        ) : isLoading ? (
              ...
              <div key={day}>
                {groupedSlots[day].length === 0 ? (
                  <div className="space-y-4">
                    ...
                    <div className="p-8 text-center rounded-4xl border-2 border-dashed border-zinc-100 dark:border-zinc-800 text-[10px] font-black text-zinc-300 uppercase tracking-widest">No Sessions for this Batch</div>
                  </div>
                ) : (
                  <TimetableDayGroup
                    day={day}
      ...
      <Modal isOpen={deleteConfirm.open} onClose={() => setDeleteConfirm({ open: false, id: null })} size="sm">
        <div className="text-center py-4 px-2">
          ...
          <p className="text-sm font-bold text-zinc-500 mb-8 px-4 leading-relaxed">Permanent removal of this entry. This cannot be undone.</p>
          <div className="flex gap-3">
            <Button variant="ghost" className="flex-1 h-12 rounded-2xl font-black uppercase text-[10px]" onClick={() => setDeleteConfirm({ open: false, id: null })}>Cancel</Button>
            <Button variant="danger" className="flex-[1.5] h-12 rounded-2xl font-black uppercase text-[10px] shadow-xl shadow-red-500/20" isLoading={deleteMutation.isPending} onClick={() => deleteMutation.mutate(deleteConfirm.id)}>Confirm Deletion</Button>
          </div>
        </div>
      </Modal>
    </div>
