...
import { Edit3, Trash2, BookOpen, GraduationCap, Search, ChevronDown, Camera, ShieldCheck, Fingerprint, UserPlus } from 'lucide-react';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { getAllStudents, deleteStudent } from '../../api/index';
...
const Students = () => {
    ...
    const getDeptName = (id) => {
        ...
        return dept ? dept.name.toUpperCase() : 'Unassigned Dept';
    };
    ...
    const formatText = (str) => str?.split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(' ');
        ...
        <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 pt-10 pb-20 px-4 md:px-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 mb-10">
                    <div className="lg:col-span-8">
                        <Input
                            ...
                            name="search"
                            icon={Search}
                            placeholder="Identify by name, Roll No or system ID..."
                            value={search}
                            onChange={setSearch}
                    ...
                    <AnimatePresence mode="popLayout">
                        {isLoading ? (
                            Array(6).fill(0).map((_, i) => (
                                <div key={i} className="h-80 rounded-[2.5rem] bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 animate-pulse" />
                            ))
                            ...
                            <motion.div
                                ...
                                className="group relative bg-white dark:bg-zinc-900 rounded-[2.5rem] border border-zinc-100 dark:border-zinc-800 p-6 transition-all hover:shadow-2xl hover:shadow-violet-600/10 hover:border-violet-200 dark:hover:border-violet-600/30 overflow-hidden"
                            >
                                <div className="flex justify-between items-start mb-6">
                                    <div className="w-20 h-20 rounded-[1.5rem] bg-zinc-50 dark:bg-zinc-800 border border-zinc-100 dark:border-zinc-700 overflow-hidden shadow-inner group-hover:border-violet-500/30 transition-colors duration-500">
                                        {student.profile_pic ? (
                                            <img src={student.profile_pic} alt="avatar" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                                        ) : (
                                            <div className="w-full h-full flex items-center justify-center text-2xl font-black text-violet-600 uppercase">
                                                {student.first_name[0]}{student.last_name[0]}
                                            </div>
                                        )}
                                    </div>
                                    <div className={`px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest flex items-center gap-2 border ${student.face_embedding
                                ...
                                <div className="space-y-1.5 mb-6">
                                    <h3 className="text-xl font-black text-zinc-900 dark:text-white leading-tight">{formatText(`${student.first_name} ${student.last_name}`)}</h3>
                                    <div className="flex items-center gap-2">
                                        <span className="text-[10px] font-black text-violet-600 uppercase tracking-widest bg-violet-50 dark:bg-violet-900/40 px-2.5 py-1 rounded-lg">
                                            Roll No: {student.roll_no}
                                        </span>
                                    </div>
                                </div>

                                <div className="bg-zinc-50/50 dark:bg-zinc-800/40 rounded-2xl p-4 border border-zinc-100 dark:border-zinc-800/50 mb-8 transition-colors group-hover:bg-white dark:group-hover:bg-zinc-800">
                                    <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-1.5">Department</p>
                                    <p className="text-xs font-bold text-zinc-800 dark:text-zinc-200 truncate tracking-tight">
                                ...
                                <div className="grid grid-cols-4 gap-2">
                                    <Button 
                                        ...
                                        className="col-span-2 h-12 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-violet-600/20"
                                    >
                                        Enroll
                                    </Button>
                                    <Button 
                                        variant="secondary" 
                                        icon={Camera} 
                                        onClick={() => openModal('face', student)}
                                        className="h-12 bg-blue-50 dark:bg-blue-500/10 text-blue-600 rounded-xl border border-blue-100 dark:border-blue-500/20 shadow-none"
                                    />
                                    <div className="flex gap-2 col-span-1">
                                        <Button 
                                            ...
                                            className="w-1/2 h-12 bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 rounded-xl border-none shadow-none"
                                        />
                                        <Button 
                                            variant="secondary" 
                                            icon={Trash2} 
                                            onClick={() => setDeleteConfirm({ open: true, id: student._id })}
                                            className="w-1/2 h-12 bg-red-50 dark:bg-red-500/10 text-red-500 rounded-xl border border-red-100 dark:border-red-500/20 shadow-none"
                                        />
                                    </div>
                                </div>
                            </motion.div>
            ...
            <Modal isOpen={deleteConfirm.open} onClose={() => setDeleteConfirm({ open: false, id: null })} size="sm">
                <div className="text-center py-4 px-2">
                    ...
                    <p className="text-sm font-bold text-zinc-500 mb-8 px-6 leading-relaxed">This will permanently delete the student profile, roll number mapping and face biometric data. This action is final.</p>
